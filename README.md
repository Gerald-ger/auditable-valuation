...
[![981 offline tests](https://img.shields.io/badge/tests-981%20offline-brightgreen.svg)](#tests)
...
| [Tests](#tests) | 981 offline, and what they are for |
...
backend\.venv\Scripts\python.exe -m pytest          # 776 tests, offline, seconds
backend\.venv\Scripts\python.exe -m pytest -m network   # live yfinance contract checks
cd frontend; npm test                                   # 205 tests
```

**981 of those run offline**, against eight companies' real financial statements committed to
this repo — 776 backend, 205 frontend, seconds. 27 more are `network`-marked and deselected by
...
- `backend/tests/fixtures/` — captured Yahoo Finance responses for ten symbols, kept because
  the 776-test suite runs entirely offline against them. Provenance and capture dates in
