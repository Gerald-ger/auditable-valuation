...
[![1050 offline tests](https://img.shields.io/badge/tests-1050%20offline-brightgreen.svg)](#tests)
...
| [Tests](#tests) | 1050 offline, and what they are for |
...
backend\.venv\Scripts\python.exe -m pytest          # 808 tests, offline, seconds
backend\.venv\Scripts\python.exe -m pytest -m network   # live yfinance contract checks
cd frontend; npm test                                   # 242 tests
```

**1050 of those run offline**, against eight companies' real financial statements committed to
this repo — 808 backend, 242 frontend, seconds. 27 more are `network`-marked and deselected by
...
- `backend/tests/fixtures/` — captured Yahoo Finance responses for ten symbols, kept because
  the 808-test suite runs entirely offline against them. Provenance and capture dates in
