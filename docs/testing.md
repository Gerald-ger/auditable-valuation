...
Of the 788 collected, 27 are `network`-marked and deselected by default. One more skips
unless OpenBB is installed — it checks that the settings path `comps.py` computes still
matches OpenBB’s own constant, which is unanswerable without it, so CI reports 760 and a skip.
