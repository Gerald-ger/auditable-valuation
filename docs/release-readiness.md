...
**What it does not do is lower the barrier to *start*.** It still needs Python 3.12 or newer, the
...
| ~~Relaxing Python below 3.14~~ | ~~The floor is `requires-python = ">=3.14"`, chosen to match the only tested configuration — **not** a dependency constraint. `pandas==3.0.5` ships wheels back to cp311 and declares `>=3.11`; `numpy==2.5.1` ships back to cp312 and declares `>=3.12`. So 3.12/3.13 would plausibly work; lowering the floor honestly means running the suite there first, which is a task, not a config edit~~ — **done 2026-08-28.** The task was carried out exactly as written: 3.12.13 and 3.13.14 each installed all 107 runtime pins and passed all 667 offline tests, and five endpoints across eight companies returned **2,288 fields with zero differences** against 3.14. The floor is now `>=3.12`, which is where `numpy` puts it, and [ci.yml](../.github/workflows/ci.yml) runs all three so it stays true. The analysis in the struck-through text was correct; only its conclusion was too cautious. | — |
| ~~CI OS / version matrix~~ | ~~One Linux runner already catches import and logic regressions~~ — **done 2026-08-28**, in two steps on the same day: three OSes (the trigger on the right fired — Windows is where this is developed and was the only platform never tested), then three interpreters on ubuntu. Eight legs, `fail-fast: false`. | — |
